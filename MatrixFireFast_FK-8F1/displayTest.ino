...
void displayTest(){
  ...
  for ( uint16_t i=0; i<cols; ++i ) { // pix[r][c]
    pix[0][i] = NCOLORS-1;
    matrix.drawPixel(0,i,color24bitTo565(flameColor[pix[0][i]]));
  ...
  for ( uint16_t i=0; i<rows; ++i ) {
    pix[i][0] = NCOLORS-1;
    matrix.drawPixel(i,0,color24bitTo565(flameColor[pix[i][0]]));
    matrix.show(); 
    delay(1000/FPS);
  ...
  for(int i=NCOLORS-1; i>=0; i--){
    if(r < rows){  // pix[r][c]
      pix[r][0] = i;
      matrix.drawPixel(r,0,color24bitTo565(flameColor[pix[r][0]]));
      pix[r][cols-1] = i;
      matrix.drawPixel(r,cols-1,color24bitTo565(flameColor[pix[r][cols-1]]));
      r++;
    }
    else break;
  }
